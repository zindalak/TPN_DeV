...
def get_neg_c_triple_gamma_llh(eval_network_doms_and_track_fn):
    ...
    def neg_c_triple_gamma_llh(track_direction,
        ...
        #logits, av, bv, geo_time, closest_approach = eval_network_doms_and_track_fn(dom_pos, track_vertex, track_direction)
        logits, av, bv, geo_time = eval_network_doms_and_track_fn(dom_pos, track_vertex, track_direction)
        #max_charges = 3.6*jnp.exp(0.23*closest_approach)+1
        #n_photons = jnp.clip(charges, min=1.0, max=max_charges)
        #n_photons = jnp.clip(n_photons, max=1000.0)
