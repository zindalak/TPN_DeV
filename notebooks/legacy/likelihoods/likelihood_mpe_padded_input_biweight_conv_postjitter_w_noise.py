...
def get_neg_c_triple_gamma_llh(eval_network_doms_and_track_fn):
    ...
    def neg_c_triple_gamma_llh(track_direction,
        ...
        X_safe = jnp.array(7.0) # when to stop evaluating negative time residuals in units of sigma
        ...
        safe_delay_time = jnp.where(delay_time > -X_safe * sigma_post, delay_time, -X_safe * sigma_post)
        physics_probs = postjitter_c_mpe_biweight_v(safe_delay_time, mix_probs, av, bv, n_photons, sigma, sigma_post)

        noise_probs = norm_pdf(delay_time, 0.0, scale=sigma_noise)
        #noise_probs = c_multi_gamma_spe_prob_large_sigma_fine_v(delay_time,
        #        mix_probs,
        #        av,
        #        bv,
        #        sigma_noise)
