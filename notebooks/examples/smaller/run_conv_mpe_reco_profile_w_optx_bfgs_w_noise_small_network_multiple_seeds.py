...
seeds = jnp.array(get_vert_seeds(centered_track_pos, track_src, v_ax=[20.0, 20.0], r_ax=[20.0]))
...
solver = optx.BestSoFarMinimiser(optx.BFGS(rtol=1e-8, atol=1e-4, use_inverse=True))
...
def run_3D(track_dir, x0):
    #x0 = jnp.array(centered_track_pos/scale)
    result = optx.minimise(neg_llh_3D, solver, x0, args=track_dir, throw=False)
    values = result.value
    return neg_llh_3D(values, track_dir), result.result._value

run_3D_v = jax.jit(jax.vmap(run_3D, (0, None), (0, 0)))
...
for x0 in seeds:
    tic = time.time()
    logls_, stat = run_3D_v(init_dirs, x0)
    logls.append(logls_)
    status.append(stat)
    toc = time.time()
...
logls = jnp.stack(logls, axis=0)
logls = jnp.nanmin(logls, axis=0)
logls = logls.reshape(X.shape)

status = jnp.stack(status, axis=0)
status = jnp.min(status, axis=0)
print(status)
