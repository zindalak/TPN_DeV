...
# Make some number of CPU cores visible.
...
# Event Index within File (i<100).
event_index = int(sys.argv[2])

# Get network and eval logic.
dtype = jnp.float64
eval_network_v = get_network_eval_v_fn(bpath='/home/storage/hans/jax_reco_new/data/smaller_network', dtype=dtype)
...
def run_3D(track_dir):
    # track_dir is a single direction.
...
@jax.jit
def run_3D_mapped(track_dir):
    # track_dir is a vector (size N) of directions.
    logls = jax.lax.map(run_3D, track_dir)
    return logls
...
if n_cpu == 1:
    logls = 1.e5 * np.ones(len(init_dirs))
    tic = time.time()
    ...
    for i, d in enumerate(init_dirs):
        ...
        if i% 10 == 0:
            toc = time.time()
            print(f"jit + reco of {i} grid points took {toc-tic:.1f}s.")

    # could also use jax's internal mapping function instead of the loop.
    #logls = run_3D_mapped(init_dirs)
    #logls.block_until_ready()

    toc = time.time()

else:
    # Need some more magic for cpu parallelism.
    # Need to pad grid such that it is divisible by n_cpu
    # Run everything in parallel
    # And then throw out the padded portion from the answer.
    # Arbitraryily chose to pad with best-fit direction points.
    tic = time.time()
    run_3D_p = jax.pmap(run_3D_mapped,
                        in_axes=0,
                        out_axes=0,
                        devices=jax.devices("cpu")[:n_cpu])

    orig_len = init_dirs.shape[0]
    multiple, remaining = divmod(orig_len, n_cpu)
    n_padded = n_cpu - remaining
    padded_dirs = jnp.repeat(jnp.expand_dims(best_dir, axis=0), n_padded, axis=0)
    init_dirs = jnp.concatenate([init_dirs, padded_dirs], axis=0)
    if n_padded > 0:
        multiple += 1
    init_dirs = init_dirs.reshape((n_cpu, multiple, init_dirs.shape[1]))
    logls = jnp.concatenate(run_3D_p(init_dirs), axis=0)[:orig_len]
    logls.block_until_ready()
    toc = time.time()
    print(f"finished parallel processing. time spent: {toc-tic:.1f}s.")
...
plt.legend()
plt.tight_layout()
plt.savefig(f"mpe_scan_ev_{event_id}_idx_{event_index}_w_noise_cpu.png", dpi=300)
