...
from likelihood_spe_w_noise import get_neg_c_triple_gamma_llh
...
eval_network_v = get_network_eval_v_fn(bpath='/home/storage/hans/jax_reco_new/data/network', dtype=jnp.float64)
...
result = optx.minimise(neg_llh_5D, solver, x0, throw=False)
best_x = result.value
...
print(optx.RESULTS[result.result])
print(result.result._value)
...
plt.legend()
plt.tight_layout()
plt.savefig(f"spe_scan_ev_{event_index}_w_noise.png", dpi=300)
