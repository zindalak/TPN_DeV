...
loss_hists = cloudpickle.load(open("/home/storage/hans/suppress_stochastics/code2/direct_network/plots/loss_hists_ftpv1_npe/loss_hists.cpkl", "rb"))
...
def make_event_plot(event_id):
    ...
    bp = '/home/storage2/hans/i3files/alerts/bfrv2_no_hole_ice_flat/ftr/charge/'
    ...
    #if os.path.exists(f"pdfs_{event_id}.pdf"):
    #    # no need to redo now
    #    return
            ...
            for j in range(n_doms_per_page):
                ...
                for k in range(3):
                    ...
                    if k == 0:
                        ...
                        _cts, _, _ = tax.hist(np.array(dom_data[pos][time_key]),
                        ...
                        tax.set_ylim([0.0, 1.2*np.amax([np.amax(yval2) / norm2, np.amax(_cts)])])
                        ...
                        dz = (float(dist)/1.e3, float(z)/1.e3)
                        key = f"{dz[0]:.5f}_{dz[1]:.5f}"
                        print(key)
                        if key not in loss_hists[event_id]:
                            print("missing energy loss histogram for this dom.")
                            continue

                        new_edges, preds = loss_hists[event_id][key]
                        tax.hist(0.5*(new_edges[1:]+new_edges[:-1]), bins=new_edges, weights=preds)
                        tax.set_yscale('log')
                        tax.set_xlabel("dist to loss [m]", fontsize=10)
                        tax.set_ylabel("Qpred [pe]", fontsize=10)
                        tax.axvline(x=0, color='black', linestyle='dashed', lw=0.5)
                        tax.set_ylim([1.e-1, 1.e3])
                        tax.set_xlim([-150, 150])
